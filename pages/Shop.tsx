...
const Shop: React.FC<ShopProps> = ({ onBack, products, coins, appSettings }) => {
    ...
    const triggerCoinFly = (coin: CoinType, startRect?: DOMRect) => {
        return new Promise<void>((resolve) => {
            ...
            const padding = 20;
            const targetX = trayRect.left + padding + Math.random() * (trayRect.width - coinWidth - padding * 2);
            const targetY = trayRect.top + padding + Math.random() * (trayRect.height - coinHeight - padding * 2);
    ...
    const handleWalletClick = (coin: CoinType, e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (stage === 'SETUP') {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center p-6 space-y-8 animate-fade-in">
                <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl w-full max-w-2xl border-t-8 border-blue-500">
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {RANGES.map(opt => (
                                    <button
                                        ...
                                        className={`p-3 rounded-xl font-bold border-2 text-sm md:text-base transition-all ${selectedRangeId === opt.id ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-slate-500 border-slate-200'}`}
                                    >
                                        {opt.label}
                                    </button>
                                ))}
                        ...
                        <label className="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-slate-50">
                            <input type="checkbox" checked={enableChangeMode} onChange={e => setEnableChangeMode(e.target.checked)} className="w-6 h-6" />
                            <span className="ml-3 text-lg font-bold text-slate-700">開啟「找錢練習」模式</span>
                        </label>
                        <div className="flex gap-4 pt-4">
                            <button onClick={onBack} className="flex-1 py-4 rounded-xl bg-gray-200 text-gray-700 font-bold text-xl">返回</button>
                            <button onClick={startGame} className="flex-1 py-4 rounded-xl bg-blue-500 text-white font-bold text-xl shadow-lg">開始購物</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="w-full h-full flex flex-col bg-slate-100 relative">
             ...
             <div className="bg-blue-600 p-2 md:p-4 text-white flex items-center shadow z-20">
                <button onClick={() => setStage('SETUP')} disabled={isAnimating} className="px-4 py-2 hover:bg-white/20 rounded-lg font-bold">設定</button>
                <div className="flex-1 text-center font-bold text-lg md:text-2xl">{stage === 'CHANGE_ACTION' ? '請拿走要付的錢' : '購物結帳'}</div>
                <button onClick={() => setEnableVoiceFeedback(!enableVoiceFeedback)} className={`p-2 rounded-full ${enableVoiceFeedback ? 'bg-yellow-400 text-blue-900' : 'bg-blue-800 text-gray-400'}`}>
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                <div className="flex-1 flex flex-col p-4 space-y-4 overflow-y-auto">
                    <div className="bg-white rounded-2xl p-4 shadow-lg flex items-center relative overflow-hidden shrink-0">
                        <img src={currentProduct?.image} alt={currentProduct?.name} className="w-24 h-24 md:w-32 md:h-32 object-cover rounded-xl" />
                        ...
                        {stage === 'BUYING' && !isAnimating && (
                             <button onClick={magicSolve} className="absolute top-2 right-2 p-2 text-yellow-400"><i className="fas fa-magic text-2xl"></i></button>
                        )}
                    </div>
                    {feedbackMessage && <div className="bg-yellow-100 text-yellow-800 p-3 rounded-lg text-center font-bold">{feedbackMessage}</div>}
                    <div ref={trayRef} className="flex-1 bg-slate-200 rounded-3xl border-4 border-slate-300 border-dashed relative min-h-[250px] flex flex-col">
                        <div className="absolute top-0 right-0 bg-slate-800 text-white px-4 py-2 rounded-bl-xl font-mono text-xl">
                            {stage === 'CHANGE_ACTION' ? `剩餘: ${trayCoins.reduce((s,c)=>s+c.value,0)}` : `總額: ${trayTotal}`}
                        </div>
                        <div className="flex-1 p-8 flex flex-wrap content-center justify-center gap-4">
                            {trayCoins.map((coin, idx) => <Coin key={idx} data={coin} onClick={() => handleTrayClick(idx)} className="animate-fade-in-up" />)}
                        </div>
                        <div className="p-4 bg-slate-300 rounded-b-2xl flex gap-4">
                            {stage === 'BUYING' ? (
                                <>
                                    <button onClick={resetTray} disabled={isAnimating} className="flex-1 py-3 bg-white text-slate-600 rounded-xl font-bold shadow">重來</button>
                                    <button onClick={checkPayment} disabled={isAnimating} className="flex-[2] py-3 bg-green-500 text-white rounded-xl font-bold shadow-lg text-xl">付錢</button>
                                </>
                            ) : (
                                <div className="w-full text-center text-blue-800 font-bold">請拿走要付的 ${currentProduct?.price} 元</div>
                            )}
                        </div>
                    </div>
                </div>
                <div className={`bg-white shadow-2xl z-10 md:w-80 md:border-l flex flex-col ${stage !== 'BUYING' ? 'hidden md:flex md:opacity-50' : ''}`}>
                    <div className="p-4 bg-slate-800 text-white font-bold">我的錢包</div>
                    <div className="flex-1 p-4 overflow-y-auto grid grid-cols-3 md:grid-cols-2 gap-4">
                        {coins.map(coin => (
                            <div key={coin.value} id={`wallet-coin-${coin.value}`} className="flex flex-col items-center">
                                <Coin data={coin} onClick={(e) => handleWalletClick(coin, e)} className={`mb-2 ${isAnimating ? 'opacity-50' : ''}`} size="md" />
