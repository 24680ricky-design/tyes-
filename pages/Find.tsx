...
const Find: React.FC<FindProps> = ({ onBack, coins, appSettings }) => {
    ...
    // Initialize with all coins selected by default
    useEffect(() => {
    ...
    const generateRound = useCallback(() => {
        ...
        // 4. Generate scattered items
        const items = [];
        const count = 8; // Total items
    ...
    if (stage === 'SETUP') {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center p-6 bg-green-50 animate-fade-in">
                <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh]">
                            ...
                            {coins.map(coin => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            relative p-4 rounded-xl border-4 transition-all duration-200 flex flex-col items-center
                                            ${isSelected 
                                                ? 'border-green-500 bg-green-50 scale-105 shadow-md' 
                                                : 'border-slate-100 bg-white opacity-60 hover:opacity-100 hover:border-green-200'
                                            }
                                        `}
                                    >
                    ...
                    <div className="pt-6 mt-4 border-t flex gap-4">
                        ...
                        <button 
                            ...
                            className={`
                                flex-[2] py-4 rounded-xl font-bold text-xl shadow-lg transition-colors flex items-center justify-center
                                ${selectedValues.length === 0 
                                    ? 'bg-slate-300 text-slate-500 cursor-not-allowed' 
                                    : 'bg-green-500 text-white hover:bg-green-600 btn-press'
                                }
                            `}
                        >
    ...
    return (
        <div className="w-full h-full flex flex-col bg-green-50 overflow-hidden relative">
             {/* Header */}
             <div className="bg-green-500 p-4 text-white flex items-center shadow-lg z-20">
                <button onClick={() => setStage('SETUP')} className="px-4 py-2 hover:bg-white/20 rounded-lg font-bold transition-colors flex items-center">
                    <i className="fas fa-arrow-left mr-2"></i> 設定
                </button>
                <h1 className="text-2xl font-bold ml-4 flex-1 text-center md:text-left">找找看</h1>
                <button onClick={replayQuestion} className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg font-bold">
            ...
            <div className="flex-1 relative w-full h-full">
                ...
                <div className="absolute top-4 left-0 right-0 text-center z-10 pointer-events-none">
                    <div className="inline-block bg-white/90 backdrop-blur px-8 py-3 rounded-full shadow-lg border-2 border-green-200">
                        <span className="text-2xl text-green-800 font-bold">請找出：</span>
                        <span className="text-3xl text-green-600 font-bold ml-2">{targetCoin?.label}</span>
                    </div>
                </div>
                ...
                {showSuccess && (
                    <div className="absolute inset-0 z-30 flex items-center justify-center bg-green-500/20 backdrop-blur-sm animate-fade-in">
                        <div className="bg-white p-8 rounded-3xl shadow-2xl text-center transform scale-110">
                            <i className="fas fa-check-circle text-8xl text-green-500 mb-4 animate-bounce"></i>
                            <h2 className="text-4xl font-bold text-slate-800">答對了！</h2>
                        </div>
                    </div>
                )}
            </div>
